...
void File::fileName(void)
{
    cout<<"Write the name on the file (without .obj) :";
        ...
        if(val.size())
        {
            if(val[0]=="f")
            {
                ...
                for(int i = 1; i < val.size(); i++)
                    f.addVertex(std::stoi(val[i])-1); // -1 porque en el archivo no empieza en 0, sino en uno entonces hay que "traducirlo" de forma que c++ lo entienda
                figure->addFace(f);
            }
            else
                if(val[0] == "v")
